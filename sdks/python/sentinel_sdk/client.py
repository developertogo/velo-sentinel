...
class SentinelClient:
    """
    High-performance Python client for Velo-Sentinel Inference Gateway.
    Handles gRPC connection pooling and provides a simplified API.
    """
    ...
    def infer(self, value, model_name="simple", session_id="python-client", timeout=1.0):
        ...
        try:
            ...
            # Extract result from raw output
            # For simplicity in this demo, we assume the first output is our float
            if response.raw_output_contents:
                import struct
                raw_data = response.raw_output_contents[0]
                result = struct.unpack('<f', raw_data)[0]
                return {
                    "result": result,
                    "latency_ms": duration,
                    "model": model_name
